...
public class GeneradorSudokuTest {
    ...
    void generar_Facil_GeneraTableroConMenosHuecos() {
        ...
        assertTrue(vacias >= 30 && vacias <= 45, "Debe tener aproximadamente 35 celdas vacías");
    }
    ...
    void generar_Dificil_GeneraTableroConMasHuecos() {
        ...
        long vacias = contarVacias(tablero);
        assertTrue(vacias >= 50 && vacias <= 65, "Debe tener aproximadamente 60 celdas vacías");
    }
    ...
    void generar_ConDificultadInvalida_UsaPorDefecto() {
        ...
        long vacias = contarVacias(tablero);
        assertTrue(vacias >= 30 && vacias <= 45, "Debe usar dificultad por defecto (fácil)");
    }

    @Test
    void esSeguro_DevuelveTrueSiNoHayConflictos() throws Exception {
        int[][] grid = new int[9][9];
        Method metodo = GeneradorSudoku.class.getDeclaredMethod("esSeguro", int[][].class, int.class, int.class, int.class);
    ...
    void rellenarSolucion_DevuelveGridCompletoYValido() throws Exception {
        int[][] grid = new int[9][9];
        Method metodo = GeneradorSudoku.class.getDeclaredMethod("rellenarSolucion", int[][].class);
        metodo.setAccessible(true);
        ...
        for (int[] fila : grid) {
            for (int val : fila) {
                assertTrue(val >= 1 && val <= 9, "Valor inválido en el tablero: " + val);
            }
        }
    }
    ...
    void contarSoluciones_UnicaSolucionParaGridCompleto() throws Exception {
        int[][] grid = new int[9][9];
        Method metodoRellenar = GeneradorSudoku.class.getDeclaredMethod("rellenarSolucion", int[][].class);
        metodoRellenar.setAccessible(true);
        boolean success = (boolean) metodoRellenar.invoke(generador, new Object[]{grid});
        assertTrue(success);

        Method metodoContar = GeneradorSudoku.class.getDeclaredMethod("contarSoluciones", int[][].class);
        metodoContar.setAccessible(true);
        int soluciones = (int) metodoContar.invoke(generador, new Object[]{grid});
        assertEquals(1, soluciones, "Un tablero completo y válido debe tener exactamente una solución");
    }
